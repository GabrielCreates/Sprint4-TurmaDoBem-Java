...
public class DoacaoResource {
    ...
    public Response cadastrar(Doacao doacao) {
        try {
            new DoacaoBO().registrarComValidacao(doacao);
    ...
    @PUT
    @Path("/{id}")
    public Response atualizar(@PathParam("id") int id, Doacao doacao) throws SQLException, ClassNotFoundException {
    ...
    public Response deletar(@PathParam("id") int id) throws SQLException, ClassNotFoundException {
        ...
        if (existente == null) return Response.status(404).entity("Doacao nao encontrada").build();
        bo.deletar(id);
        return Response.ok("Doacao removida").build();
    }
}